...
class GAMCR(Dataset, Trainer):
    ...
    def train(self, X, matJ, Y, dates = None, lr=1e-3, max_iter=200, warm_start=False, save_folder=None, name_model='', normalization_loss=1):
        ls_X = [X for l in range(self.L)]
        ls_modelmat = [matJ[:,l,:] for l in range(self.L)]
        loss_curve = self.trainer(ls_X, ls_modelmat, Y, dates=dates, lr=lr, max_iter=max_iter, warm_start=warm_start, save_folder=save_folder, name_model=name_model, normalization_loss=normalization_loss)
