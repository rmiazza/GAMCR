...
class ComputeStatistics():
    ...
    def compute_statistics(self, site_folder, site, nblocks = 4, min_precip = 1, groups_wetness=None, groups_precip=None, max_files=20, normalization_streamflow=1, save_folder=None, filtering_time_points=None):
        ...
        for k in range(K):
            df_transfer['predicted_NRF_group_{0}'.format(k)] = H_weighted_avg[k,:]
            df_transfer['predicted_RRD_group_{0}'.format(k)] = H_avg[k,:]
            df_transfer['predicted_weighted_avg_RRD_group_{0}'.format(k)] = weighted_avg_RRD[k,:]
        if true_tfs:
            for k in range(K):
                df_transfer['true_NRF_group_{0}'.format(k)] = H_weighted_avg_true[k,:]
                df_transfer['true_RRD_group_{0}'.format(k)] = H_avg_true[k,:]
                df_transfer['true_weighted_avg_RRD_group_{0}'.format(k)] = weighted_avg_RRD_true[k,:]
        ...
        if true_tfs:
            ...
            with open(os.path.join(save_folder_detailssave_folder_details, 'stats_NRF_true.pkl'), 'wb') as handle:
                pickle.dump(stats_true, handle, protocol=pickle.HIGHEST_PROTOCOL)
            with open(os.path.join(save_folder_detailssave_folder_details, 'stats_RRD_true.pkl'), 'wb') as handle:
