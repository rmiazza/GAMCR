...
class Dataset():
    """
    Preprocess, load, and manage data and models for the GAMCR framework.

    This class handles the preparation of input data (precipitation, PET,
    streamflow), construction of GAM design matrices, convolution operators,
    and spline bases. It also supports saving and loading preprocessed data
    ...
    save_batch_common_GAM(allGISID, save_folder, ntest=0, nstart=0, nfiles=40)
        Preprocess and save the data. It makes sure to use the same knots for
        the GAMs for the different sites.
    ...
    get_design(pet, x, y, dates)
        Compute the design matrix of the GAMs. This method is called by the
        'save_batch' type methods.
    get_GAMdesign(X, J)
        Compute the matrix that is used in the convolution to get the
        streamflow values.
    load_data(save_folder, max_files=100, test_mode=False)
        Load the data that has been already proprecessed and saved using one
        of the 'save_batch' type methods.
    compute_spline_basis(show_splines = False)
        Compute the basis functions (B-splines) one which we decompose the
        transfer functions.
    """
    def __init__(self, max_lag=24*30*2, features={}, n_splines=10,  lam=10):
        """Initialize dataset attributes and associated GAM model."""
        self.m = max_lag

        # Compute spline basis for transfer function decomposition
        ...
        # Initialize the GAM model associated with this dataset
    ...
    def load_model(self, path_model, lam=None):
        ...
        lam : positive float, optional
            Regularization parameter for the smoothing penalty used when
            fitting the GAM
        """
        ...
        self.gam.init_gam_from_knots(
            params['gam']['edge_knots_'],
            params['gam']['m_features'],
            coeffs=params['gam'].get('coeffs', None))
    ...
    def save_batch_common_GAM(self, allGISID, save_folder, ntest=0, nstart=0, nfiles=40):
        ...
        It makes sure to use the same knots for the GAMs for the different sites.
        This was needed in our paper when trying to fit a model predicting model
        coefficients from catchment's features. Otherwise, coefficients learned
        at different sites would not correspond to the same quantities.
        ...
        save_folder : str
            Path where we can find a folder '{site}' for all '{site}' in allGISID.
            In this folder, the .txt file with the data at the corresponding site
            should be saved.
        ntest : int, optional
        ...
        nfiles : int, optional
            The preprocessed data will be splitted and saved in different files
            (to potentially speed up the loading process of the data if only some
            fraction of the total dataset is needed). This integer specifies the
            number of files used to split the data.
        """
        init = self.init
        m = self.m
        common_edge_knots = None

        for gis_id in allGISID:
            path_root = os.path.join(save_folder, str(gis_id))
            path = os.path.join(path_root, 'data/')

            if not os.path.exists(path):
                os.makedirs(path)
            pathfile = os.path.join(path_root, 'data_{0}.txt'.format(gis_id))

            (pet_train, x_train, y_train, dates_train, timeyear_train,
             pet_test, x_test, y_test, dates_test, timeyear_test) = self.get_fluxes(pathfile, ntest=ntest, nstart=nstart)
        ...
        for gis_id in allGISID:
            ...
            for l in range(nfiles-1):
                ...
                np.save(os.path.join(path, f'matJ_{l}.npy'), matJ[low:up, :, :])
                np.save(os.path.join(path, f'X_{l}.npy'), X[low:up, :])
                np.save(os.path.join(path, f'y_{l}.npy'), Y[low:up])
                np.save(os.path.join(path, f'indexes_{l}.npy'), np.array([i for i in range(low, up)]))
                np.save(os.path.join(path, f'timeyear_{l}.npy'), np.array(timeyear[low:up]))
                np.save(os.path.join(path, f'dates_{l}.npy'), np.array(dates[low:up]))

            self.save_model_parameters(path)

    def save_batch(self, save_folder, datafile, nstart=0, nfiles=100):
        """Preprocess and save model input data in multiple batches for al
        sites/catchments.
    ...
    def get_design(self, pet, x, y, dates):
        ...
        for el in ages_max_x:
            self.feature_names.append(f'Weighted avg precipitation over the last {el} hours')
        for el in ages_max_pet:
            self.feature_names.append(f'Weighted avg pet over the last {el} hours')
    ...
    def load_data(self, save_folder, max_files=100, test_mode=False):
        ...
        max_files : int
            Total number of files among the ones saved by the 'save_batch'
            type method to load.
        test_mode : bool
            If False, the oldest data will be loaded. Otherwise, the more
        ...
        for ite, id_file in enumerate(id_files_2_load):
            if ite == 0:
                X = np.load(os.path.join(save_folder, f'X_{id_file}.npy'))
                matJ = np.load(os.path.join(save_folder, f'matJ_{id_file}.npy'))
                y = np.load(os.path.join(save_folder, f'y_{id_file}.npy'))
                timeyear = np.load(os.path.join(save_folder, f'timeyear_{id_file}.npy'))
                dates = np.load(os.path.join(save_folder, f'dates_{id_file}.npy'), allow_pickle=True)
            else:
                Xtemp = np.load(os.path.join(save_folder, f'X_{id_file}.npy'))
                matJtemp = np.load(os.path.join(save_folder, f'matJ_{id_file}.npy'))
                ytemp = np.load(os.path.join(save_folder, f'y_{id_file}.npy'))
                timeyeartemp = np.load(os.path.join(save_folder, f'timeyear_{id_file}.npy'))
                datestemp = np.load(os.path.join(save_folder, f'dates_{id_file}.npy'),  allow_pickle=True)
    ...
    def compute_spline_basis(self, show_splines=False):
        """
        Compute the basis functions (B-splines) one which we decompose
