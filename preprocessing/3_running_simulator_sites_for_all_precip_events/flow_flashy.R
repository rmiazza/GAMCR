...
for (id_station in 1:length(STATIONS)){
    ...
    dir.create(file.path('/mydata/watres/quentin/code/FLOW/data/simulated_data_events_updatesummer2024/', paste0(STATION, "_flashy", collapse=NULL)), showWarnings = FALSE);
    
    
    path_save = paste0('/mydata/watres/quentin/code/FLOW/data/simulated_data_events_updatesummer2024/', paste0(STATION, "_flashy/", collapse=NULL), collapse=NULL);
    ...
    if ('Basel'==STATION){ # using calibrated parameters from site with GISID 58
      ...
      if (ite==1){
        filename = paste0(path_save, "ref_flow_flashy_fluxes.txt", collapse=NULL);
        ...
        filename = paste0(paste0(path_save, as.character(idx_evts[ite]), collapse=NULL), "_flow_flashy_fluxes.txt", collapse=NULL);
